...
void main()
{
	vec2 OFFSETS0 = vec2(0, -4);
	vec2 OFFSETS1 = vec2(0, -3);
	vec2 OFFSETS2 = vec2(0, -2);
	vec2 OFFSETS3 = vec2(0, -1);
	vec2 OFFSETS4 = vec2(0, 0);
	vec2 OFFSETS5 = vec2(0, 1);
	vec2 OFFSETS6 = vec2(0, 2);
	vec2 OFFSETS7 = vec2(0, 3);
	vec2 OFFSETS8 = vec2(0, 4);
	...
	float Texel = (1.0f / ViewportDimensions.y) * (LevelOfDetail + 1);
