#include "../Header/CAddBrightPass.h"

bool CAddBrightPass::Initialize(CDevice& device, Size size, Color* clearColor, CScreenAlignedQuad* SAQ,
	std::vector <CRenderTarget*> renderTargets, ShaderProgramData data)
{
	...
	names.clear();
	names.push_back("BlurHPass");
	names.push_back("BlurVPass");
...
void CAddBrightPass::Render(CDeviceContext& deviceContext)
{
#ifdef OPEN_GL
	...
	glUniform1i(m_texturesCB.m_uniforms[2].location, m_inTextures[2]->m_texture.m_id);
	glActiveTexture(GL_TEXTURE0 + m_inTextures[2]->m_texture.m_id);
	glBindTexture(GL_TEXTURE_2D, m_inTextures[2]->m_texture.m_id);

	if (m_samplerState.m_id != 0) {
		glBindSampler(m_inTextures[2]->m_texture.m_id, m_samplerState.m_id);
	}
	else {
		glBindSampler(m_inTextures[2]->m_texture.m_id, 0);
	}
#endif
	m_screenAlignedQuad->Render(deviceContext, m_inTextures);
}

void CAddBrightPass::Clear(CDeviceContext& deviceContext)
{
	deviceContext.ClearColor(m_renderTarget, *m_clearColor);
}

void CAddBrightPass::Set(CDeviceContext& deviceContext)
{
	//Generate mipmaps
	...
	deviceContext.UpdateAddBrightData(m_dataCB, m_screenAlignedQuad->m_addBrightData);

	//Set Render Target
	deviceContext.SetRenderTarget(m_renderTarget, nullptr);
}

bool CAddBrightPass::Resize(CDevice& device, Size size)
{
#ifdef DIRECT_X
	HRESULT result;
...
void CAddBrightPass::Update()
{

}

void CAddBrightPass::Erase()
{

}

CAddBrightPass::CAddBrightPass()
{
#ifdef DIRECT_X
