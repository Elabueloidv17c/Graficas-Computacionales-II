#include "../Header/CBrightPass.h"

bool CBrightPass::Initialize(CDevice& device, Size size, Color* clearColor, CScreenAlignedQuad* SAQ,
...
void CBrightPass::Render(CDeviceContext& deviceContext)
{
#ifdef OPEN_GL
...
void CBrightPass::Clear(CDeviceContext& deviceContext)
{
	deviceContext.ClearColor(m_renderTarget, *m_clearColor);
}

void CBrightPass::Set(CDeviceContext& deviceContext)
{
	//Generate mipmaps
	...
	deviceContext.UpdateBrightData(m_dataCB, m_screenAlignedQuad->m_brightData);

	//Set Render Target
	deviceContext.SetRenderTarget(m_renderTarget, nullptr);
}

bool CBrightPass::Resize(CDevice& device, Size size)
{
#ifdef DIRECT_X
	HRESULT result;
...
void CBrightPass::Update()
{

}

void CBrightPass::Erase()
{

}

CBrightPass::CBrightPass()
{
#ifdef DIRECT_X
