...
void CManager::Initialize(WindowData data)
{
#ifdef DIRECT_X
	//Create Window
	...
	//m_renderTexture.Initialize(m_device, data.dimensions.size, Color{0.8f, 0.1f, 0.0f, 1.0f});
	...
	m_window.Initialize(m_device, data.dimensions, 0, &data.title[0], Color{ 0.0f, 0.3f, 0.6f, 1.0f }, scene);
	m_viewport.Initialize(data.dimensions);
...
void CManager::Render()
{
	...
	m_window.Clear();
	m_window.Render(m_deviceContext, *m_shaderManager.GetActiveShader(), *m_window.m_camera.GetActiveCamera(), *m_window.m_camera.GetSecundaryCamera());

	//Render user interface to see the scene information
	m_userInterface.Initframe();
	m_userInterface.SetFrame(m_window.m_scene.m_colorData, m_window.m_scene.m_lightingData, m_shaderManager.m_isVertex, m_shaderManager.m_isBlinn,
	m_window.m_scene.m_isSpotOn, m_window.m_scene.m_isSpotChanged, m_window.m_scene.m_isPointOn, m_window.m_scene.m_isPointChanged);
